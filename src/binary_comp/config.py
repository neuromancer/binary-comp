...
def _target_from_standalone(config: dict[str, Any], target: str) -> ProjectTarget:
    ...
    return ProjectTarget(
        name=target,
        original_exe=require_string(target_cfg, "original_exe", f"targets.{target}.original_exe"),
        rebuilt_exe=require_string(target_cfg, "rebuilt_exe", f"targets.{target}.rebuilt_exe"),
        map_path=require_string(target_cfg, "map", f"targets.{target}.map"),
        source_dirs=_source_dirs(target_cfg.get("source_dirs") or target_cfg.get("source_dir"),
                                 f"targets.{target}.source_dirs"),
        code_dir=optional_string(target_cfg, "code_export_dir") or optional_string(target_cfg, "code_dir"),
...
def load_project_target(config_path: str | os.PathLike[str], target: str) -> tuple[dict[str, Any], ProjectTarget]:
    ...
    if "targets" in config:
        return config, _target_from_standalone(config, target)
